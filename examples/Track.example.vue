...
<template>
    <div class="Track">
      ...
      <!--左边显示内容-->
      <div class="left_container" :style="{right:blnShowLoading==0?'10px':'300px'}">
        <!--顶部显示信息-->
        <div class="top_info">
          <div class="taginfo"> 
      ...
      <!--底部显示内容-->
      <div class="bottom_info" v-show="!blnShowHistory" :style="{right:blnShowLoading==0?'0px':'300px'}">
        <!--播放器-->
        <div class="player">
          <PointPlayer ref="mapPlayer" :data="traceData"  :range="range" @countChange="countChange" @timeChange="timeChange" />
        </div>
        <!--详细按钮-->
      ...
      <!--轨迹详细信息-->
      <div class="historyList" v-show="blnShowDetail" :style="{right:blnShowLoading==0?'10px':'300px'}">
        <!--删除按钮-->
          ...
          <div class="info" v-if="blnShowLoading==2">
            <Scroll>
              ...
              <div class="item" v-show="(serachInfo.code=='imsi' || serachInfo.code=='mac') && detailInfo.range_top10.length>0">
                ...
                <div class="child" v-for="(r,i) in detailInfo.range_top10">范围{{i+1}} <span style="float:right;"><i class="fa fa-map-marker" @click="showHeatMap(r,i)"></i></span></div>
...
<style scoped lang="less">
  ...
  .Track .right_container{padding-top:40px;padding-bottom:10px;}
  ...
  .Track .right_container .info .item .fa-map-marker:hover{color:@Font_Hover_Col;cursor:pointer;}

  .Track .historyList{
    position:absolute;top:42px;right:@rightW;left:10px;bottom:10px;
  }
