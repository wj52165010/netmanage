<!-- 分析页面组件 -->
<template>
    <div class="DeviceAnalysis">
        <div class="DeviceAnalysis_container">
            ...
            <div class="bottom_container">
                ...
                <div class="option_bar">
                    <span class="title">设备在离线:</span>

                    <div class="item">
                        <span>区域范围:</span>
                        <div style="display:inline-block;">
                            <PlaceSearch  :blnClear="true" c_searchKind="0" :microprobeType="microprobe_type" @place_res="selectArea"></PlaceSearch>
                        </div>
                    </div>
            ...
            <div class="bottom_page" :style="{top:showPage!=''?'0px':'110%'}">
                ...
                <div class="title_bar" @click="showPage=''">
                    <span>{{this.showPage=='place'?'待处理设备':this.showPage=='issue'?'问题总览':''}}</span>
                </div>
                <!--内容栏-->
                <div class="content_bar">
                    <DealtPlace ref="DealtPlace" v-show="showPage=='place'" />
                    <IssueLook ref="IssueLook" :microprobe_type="microprobe_type" :abnormal_type="netbar_abnormal_type" v-if="showPage=='issue'" />
                </div>
            </div>
        </div>
    </div>
...
export default {
  ...
  data () {
    return {
      ...
      microprobe_type:DataSource['无线'],//数据来源
    }
  },
  watch:{
  ...
  methods:{
    ...
    getCurStatusAndIssueData(){
       this.$store.dispatch(LastDevicePercentage).then(res=>{
    ...
    //导出统计场所状态率
    ExportOnlineCount(){
        tool.confirm('确定要导出设备在离线记录吗?',['确定','取消'],()=>{
            this.$store.dispatch(DeviceHisPercentageExport,{
                coll_type:this.collType,
                microprobe_type:[this.microprobe_type],
                region_range:this.regionRange,
                security_software_orgcodes:_.map(this.Selfirms,s=>s.code).join(',')
    ...
    loadIssueChart(d){
        ...
        let option = {
            ...
            yAxis: {
                type: 'category',
                data: ['设备维护','设备离线','数据离线','心跳故障',"采集波动",'程序失效'],
                splitLine:{
                    show:false
                },
                axisTick: {
...
.DeviceAnalysis{width:100%;height:100%;padding:5px;position:relative;}
.DeviceAnalysis_container{width:100%;height:100%;background-color:white;position:relative;}

@topContainerH:40%;
.DeviceAnalysis_container .top_container{width:100%;height:@topContainerH;}
@curStateW:30%;
@placeW:30%;
.DeviceAnalysis_container .top_container .curState{width:@curStateW;height:100%;float:left;}
.DeviceAnalysis_container .top_container .place{width:@placeW;height:100%;float:left;}
.DeviceAnalysis_container .top_container .issue{width:~'calc(100% - @{curStateW} - @{placeW})';height:100%;float:left;}

.DeviceAnalysis_container .bottom_container{width:100%;height:~'calc(100% - @{topContainerH})';}

.DeviceAnalysis_container .option_bar{text-align:left;padding:5px 15px;line-height:40px;}
.DeviceAnalysis_container .option_bar .item{display:inline-block;margin:2px 5px;}

.DeviceAnalysis_container .option_bar .title{font-size:20px;font-weight:600;}

.DeviceAnalysis_container .chart_container{width:100%;height:~'calc(100% - 55px)'}

.DeviceAnalysis_container .right_option_bar {float:right;}
.DeviceAnalysis_container .right_option_bar .item{display:inline-block;margin:2px 5px;}
.DeviceAnalysis_container .right_option_bar .item:hover{cursor:pointer;}
html{.TCol(~".DeviceAnalysis_container .right_option_bar .item:hover");}

html{.TCol(~".DeviceAnalysis_container .right_option_bar .active");}
...
.DeviceAnalysis_container .switchBtn{
  position:absolute;
  width: 15px;
  height: 40px;
  color: white;
...
html{.TCol(~".DeviceAnalysis_container .switchBtn",'bg');}
</style>
