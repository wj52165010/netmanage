  ...
  methods:{
    ...
    loadIssueChart(data){
        ...
        let option = {
            ...
            yAxis: {
                type: 'category',
                data: ['场所停业','场所离线','数据离线','心跳故障','采集波动','程序失效',"刷卡异常",'终端异常'],
                splitLine:{
                    show:false
                },
                axisTick: {
...
.Analysis_container{width:100%;height:100%;background-color:white;}

@topContainerH:40%;
.Analysis_container .top_container{width:100%;height:@topContainerH;}
@curStateW:30%;
@placeW:30%;
.Analysis_container .top_container .curState{width:@curStateW;height:100%;float:left;}
.Analysis_container .top_container .place{width:@placeW;height:100%;float:left;}
.Analysis_container .top_container .issue{width:~'calc(100% - @{curStateW} - @{placeW})';height:100%;float:left;}

.Analysis_container .bottom_container{width:100%;height:~'calc(100% - @{topContainerH})';}

.Analysis_container .option_bar{text-align:left;padding:5px 15px;line-height:40px;}
.Analysis_container .option_bar .item{display:inline-block;margin:2px 5px;}

.Analysis_container .option_bar .title{font-size:20px;font-weight:600;}

.Analysis_container .chart_container{width:100%;height:~'calc(100% - 55px)'}

.Analysis_container .right_option_bar {float:right;}
.Analysis_container .right_option_bar .item{display:inline-block;margin:2px 5px;}
.Analysis_container .right_option_bar .item:hover{cursor:pointer;}
html{.TCol(~".Analysis_container .right_option_bar .item:hover");}

html{.TCol(~".Analysis_container .right_option_bar .active");}
