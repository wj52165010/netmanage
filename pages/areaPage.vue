<!-- 区域管理定制页面组件 created by Chuanjiang Li 2018/07/01-->
<template>
    <div class="firmpage">
...
import {waitingHandleSite,BODY_RESIZE,getDictTables,SiteDetail,GetFirm,GetFirmList,SiteHisPercentage,regionCollColumn,RegionCollLine,RegionDeviceExport,RegionSiteExport,FirmDetectHistory,FirmDetectYesterday,FirmDetectColl,RegionDetectRange,GetRegionList} from '../store/mutation-types'
export default {
  name: 'firmPage',
  components:{
      Scroll,
  ...
  methods:{
      ...
      getDetectRange(){
        ...
        this.$store.dispatch(RegionDetectRange,{begin_time:beginTime,end_time:endTime}).then(res=>{
           if(res.msg.code!='successed')return;
      ...
      loadBarChart(data){
        ...
        for(let any of data){
           barObject.FirmName.push(any.region_name)  //所有厂商名称
      ...
      // 获取场所在线柱状图数据
      getSiteOnBarData(){
         this.$store.dispatch(regionCollColumn,this.siteSelect).then(res=>{
           if(res.msg.code!='successed')return;
      ...
      // 获取设备在线柱状图数据
      getDeviceOnBarData(){
         this.$store.dispatch(regionCollColumn,this.deviceSelect).then(res=>{
           if(res.msg.code!='successed')return;
      ...
      // 获取场所在线折线图图数据
      getSiteOnlineData(){
         this.$store.dispatch(RegionCollLine,this.siteSelect).then(res=>{
           if(res.msg.code!='successed')return;
      ...
      // 获取设备在线折线图图数据
      getDeviceOnlineData(){
         this.$store.dispatch(RegionCollLine,this.deviceSelect).then(res=>{
           if(res.msg.code!='successed')return;
    ...
    addSiteBarChart(data){
      ...
      //导出统计场所状态率
    ExportSiteCount(){
        this.$store.dispatch(RegionSiteExport,this.siteSelect).then(res=>{
    ...
    //加载设备在离线折线图
     addDeviceLineChart(data){
        ...
        for(let i=0;i<data[0].regions.length;i++){
            firmNames.push(data[0].regions[i].region_name);           //获取所有厂商名
            allData.push({
                name: data[0].regions[i].region_name,
                type: 'line',
                data: [],
                itemStyle: {
                    normal: {
        ...
        for(let i=0;i<data.length;i++){
            time.push(data[i].stat_date.substr (0,4)+"-"+data[i].stat_date.substr (4,2)+"-"+data[i].stat_date.substr (6,2));
            for(let j=0;j<data[i].regions.length;j++){
                // 判断是加载场所还是加载设备
                if(this.deviceSelect.type=='device'){
                    allData[j].data.push((data[i].regions[j].equip_online_pre*100).toFixed(2))
                }else{
                     allData[j].data.push((data[i].regions[j].netbar_online_pre*100).toFixed(2))
                }
    ...
    addDeviceBarChart(data){
        ...
        if(typeIco=="device"){     
            ...
            //console.log(data);
            for(let i=0;i<data.length;i++){
                xAxisdata.push(data[i].region_name);
    ...
    ExportDeviceCount(){
        let typeIco=this.deviceSelect.type;
        if(typeIco=="device"){     
            this.$store.dispatch(RegionDeviceExport,this.deviceSelect).then(res=>{
                if(!tool.msg(res,'','导出失败!'))return;
                window.location=res.biz_body.url;
            });
        }else{
            this.$store.dispatch(RegionSiteExport,this.deviceSelect).then(res=>{
                if(!tool.msg(res,'','导出失败!'))return;
                window.location=res.biz_body.url;
            });
        }
